package;
import flixel.FlxSprite;
import flixel.util.FlxVector;

/**
 * ...
 * @author Thunraz
 */
class DestroyableRadar extends DestroyableObject
{
    public function new(X:Float=0, Y:Float=0, state:PlayState, size:FlxVector, scale:FlxVector, health:Float)
    {
        _type = DestroyableType.Radar;
        _scale = scale;

        sprite = new FlxSprite();
        sprite.loadGraphic(AssetPaths.destroyableRadar__png, true, Std.int(size.x), Std.int(size.y));
        sprite.setGraphicSize(Std.int(_scale.x), Std.int(_scale.y));
        sprite.updateHitbox();
        
        // Add "animations" and play the normal state
        sprite.animation.add("normal",    [0, 1, 2, 3, 4, 5, 6, 7], 8, true);
        sprite.animation.add("destroyed", [8],                     30, true);
        sprite.animation.play("normal");

        _health = health;
        
        super(X, Y, _type, state);
    }
}