 ...
 * @author Thunraz
 */
class DestroyableBarrel extends DestroyableObject
{
    public function new(X:Float=0, Y:Float=0, state:PlayState, size:FlxVector, scale:FlxVector, health:Float)
    {
        _type = DestroyableType.Barrel;
        _scale = scale;

        sprite = new FlxSprite();
        sprite.loadGraphic(AssetPaths.destroyableBarrel__png, true, Std.int(size.x), Std.int(size.y));
